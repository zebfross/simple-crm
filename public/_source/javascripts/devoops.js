...
function LoadCalendarScript(callback){
	...
	if (!$.fn.moment){
		$.getScript('plugins/moment/min/moment.min.js', LoadFullCalendarScript);
	}
	else {
...
function LoadFancyboxScript(callback){
	if (!$.fn.fancybox){
		$.getScript('plugins/fancybox/source/jquery.fancybox.pack.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript('plugins/jquery-knob/dist/jquery.knob.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSparkLineScript(callback){
	if(!$.fn.sparkline){
		$.getScript('plugins/jquery.sparkline/dist/jquery.sparkline.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
